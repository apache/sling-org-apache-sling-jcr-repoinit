...
public class AclUtilTest {
    ...
    public void entryWithPartiallyMatchingPrivilegesIsContained() throws Exception {
        // validates that an exact match of the username and isAllow but with privileges partially overlapping is
        // contained
    ...
    public void compareValuesTest() throws RepositoryException {
        ...
        final Value[] a1 = new Value[] {
            vf.createValue("jcr:content", PropertyType.NAME), vf.createValue("jcr:data", PropertyType.NAME)
        };
        final Value[] a2 =
                new Value[] {vf.createValue("a", PropertyType.STRING), vf.createValue("b", PropertyType.STRING)};
        final Value[] a3 =
                new Value[] {vf.createValue("b", PropertyType.STRING), vf.createValue("c", PropertyType.STRING)};

        final Value[] a4 = new Value[] {
            vf.createValue("a", PropertyType.STRING),
            vf.createValue("b", PropertyType.STRING),
            vf.createValue("c", PropertyType.STRING)
        };

        final Value[] a5 =
                new Value[] {vf.createValue("b", PropertyType.STRING), vf.createValue("a", PropertyType.STRING)};
        final Value[] a6 = new Value[] {
            vf.createValue("b", PropertyType.STRING),
            vf.createValue("a", PropertyType.STRING),
            vf.createValue("c", PropertyType.STRING)
        };
        final Value[] a7 =
                new Value[] {vf.createValue("a", PropertyType.STRING), vf.createValue("b", PropertyType.STRING)};
        final Value[] a8 = new Value[] {
            vf.createValue("jcr:data", PropertyType.NAME), vf.createValue("jcr:content", PropertyType.NAME)
        };
        final Value[] a9 = new Value[] {
            vf.createValue("jcr:data", PropertyType.NAME), vf.createValue("jcr:content", PropertyType.STRING)
        };
        final Value[] emptyA = {};
        final Value[] emptyB = {};
    ...
    public void testSetAclForEveryone() throws Exception {
        ...
        try {
            ...
            assertIsContained(
                    AccessControlUtils.getAccessControlList(session, PathUtils.ROOT_PATH),
    ...
    public void testSetAclForPrincipalNameDifferentFromId() throws Exception {
        ...
        try {
            ...
            assertIsContained(
                    AccessControlUtils.getAccessControlList(U.adminSession, PathUtils.ROOT_PATH),
                    principal.getName(),
                    new String[] {Privilege.JCR_READ},
                    false);
        } finally {
    ...
    public void testSetAclWithHomePath() throws Exception {
        ...
        assertIsNotContained(
                AccessControlUtils.getAccessControlList(U.adminSession, userHomePath),
                U.username,
                new String[] {Privilege.JCR_READ},
                true);
    ...
    public void testSetAclWithHomePathMultipleIds() throws Exception {
        ...
        assertIsNotContained(
                AccessControlUtils.getAccessControlList(U.adminSession, groupHomePath),
                U.username,
                new String[] {Privilege.JCR_READ},
                true);
    ...
    public void testSetAclWithHomePathMultiplePath() throws Exception {
        ...
        assertIsContained(
                ...
                true);
        assertIsContained(
                AccessControlUtils.getAccessControlList(U.adminSession, null),
                U.username,
                new String[] {Privilege.JCR_ALL},
                true);
        assertIsContained(
    ...
    public void testSetAclWithHomePathAndSubtree() throws Exception {
        UserManager uMgr = ((JackrabbitSession) U.adminSession).getUserManager();
        String userHomePath = U.adminSession
                .getNode(uMgr.getAuthorizable(U.username).getPath())
                .addNode("profiles")
                .addNode("private")
                .getPath();
        assertIsNotContained(
        ...
        Group gr = uMgr.createGroup("groupId");
        String groupHomePath = U.adminSession
                .getNode(gr.getPath())
                .addNode("profiles")
                .addNode("private")
                .getPath();
    ...
    private void assertIsContained(
            JackrabbitAccessControlList acl, String username, String[] privilegeNames, boolean isAllow)
            throws RepositoryException {
        assertIsContained0(acl, username, privilegeNames, isAllow, true);
    }

    private void assertIsNotContained(
            JackrabbitAccessControlList acl, String username, String[] privilegeNames, boolean isAllow)
            throws RepositoryException {
        assertIsContained0(acl, username, privilegeNames, isAllow, false);
    }

    private void assertIsContained0(
        ...
        if (contained) {
            assertTrue(
                    "ACL does not contain an entry for " + localAce,
                    AclUtil.contains(acl.getAccessControlEntries(), localAce));
        } else {
            assertFalse(
                    "ACL contains an entry for " + localAce, AclUtil.contains(acl.getAccessControlEntries(), localAce));
        }
    }
